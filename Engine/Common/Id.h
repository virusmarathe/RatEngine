...
namespace rat::id
{
	...
	{
		return (id & internal::generation_mask) >> internal::index_bits;
	}

	constexpr id_type new_generation(id_type id)
	{
		const id_type new_gen{id::generation(id) + 1};
		assert(new_gen < (((u64)1 << internal::generation_bits) - 1));
		return index(id) | (new_gen << internal::index_bits);
	}
